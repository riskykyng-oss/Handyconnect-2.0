    ...
    toast(
      () => (
        <div className="text-sm">
          ...
          <p className="mt-1 text-hc-ink-2 text-xs">For demonstration purposes only.</p>
        </div>
      ),
      { icon: '📱', duration: 15000 }
    );
        ...
        {open && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            {![STEP.processing, STEP.success].includes(step) && (
                <button
                  onClick={reset}
                  className="absolute right-3 top-3 rounded-full p-1.5 text-hc-ink-3 transition-colors hover:bg-hc-hairline hover:text-hc-accent-strong"
                  aria-label="Close"
                >
                  <X size={16} />
                </button>
              ...
              {/* Header */}
              <div className="flex items-center gap-3 border-b border-hc-hairline px-5 py-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0a4d3c]">
                  <Smartphone size={18} className="text-white" />
                </div>
                <div>
                  <p className="text-sm font-bold text-hc-ink">Simulated EcoCash Gateway</p>
                  <p className="text-[11px] text-hc-ink-3">Academic demonstration — no real transaction</p>
                </div>
              </div>

              {/* Amount */}
              <div className="border-b border-hc-hairline bg-hc-page px-5 py-3 text-center">
                <p className="text-[10px] font-medium uppercase tracking-wider text-hc-ink-3">Amount</p>
                <p className="mt-0.5 font-display text-2xl font-bold text-hc-ink">${value.toFixed(2)}</p>
                <p className="text-xs text-hc-ink-2">to {recipientName || 'Professional'}</p>
              </div>
              ...
              <div className="px-5 py-5">
                ...
                {step === STEP.phone && (
                  <div>
                    <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-hc-ink-3">
                      EcoCash Phone Number
                    </label>
                    <div className="flex items-center gap-2 rounded-xl border border-hc-hairline bg-white px-3 py-2.5 transition-all focus-within:border-[#0a4d3c] focus-within:ring-2 focus-within:ring-[#0a4d3c]/10">
                      <Phone size={16} className="shrink-0 text-hc-ink-3" />
                      <span className="text-sm font-semibold text-hc-ink-2">+263</span>
                      <input
                        value={phone}
                        onChange={(e) => { setPhone(formatPhone(e.target.value)); setPhoneError(''); }}
                        placeholder="7X XXX XXXX"
                        autoFocus
                        className="w-full bg-transparent text-sm text-hc-ink outline-none placeholder:text-hc-ink-3"
                      />
                    </div>
                    {phoneError && (
                ...
                {step === STEP.ussd && (
                  <div>
                    ...
                    <p className="mt-2 text-center text-[11px] text-hc-ink-3">
                ...
                {step === STEP.otp && (
                  <div>
                    <p className="mb-3 text-sm text-hc-accent-strong">
                      Enter the 6-digit code sent to <span className="font-semibold text-hc-ink">{fullPhone}</span>
                    </p>
                    <div className="rounded-xl bg-hc-accent-50 border border-hc-accent-200 p-3 text-center mb-4">
                      <p className="text-xs text-hc-accent-strong">
                        <span className="font-bold">Demo OTP:</span>{' '}
                        <span className="font-mono text-base font-bold text-hc-accent-800">{generatedOtp}</span>
                      </p>
                      <p className="mt-0.5 text-[10px] text-hc-accent-400">Displayed for demonstration purposes</p>
                    </div>
                    <input
                      value={otp}
                      ...
                      maxLength={6}
                      autoFocus
                      className="w-full rounded-xl border border-hc-hairline bg-hc-page px-4 py-3 text-center font-mono text-2xl tracking-[0.4em] text-hc-ink outline-none transition-all focus:border-[#0a4d3c] focus:bg-white focus:ring-2 focus:ring-[#0a4d3c]/10"
                    />
                    <button
                    ...
                    <button
                      ...
                      className="mt-2 w-full rounded-xl px-4 py-2 text-xs font-semibold text-hc-ink-2 transition-colors hover:bg-hc-hairline"
                    >
                      Change number
                    </button>
                  </div>
                ...
                {step === STEP.processing && (
                  <div className="py-4">
                    <p className="mb-4 text-center text-sm font-semibold text-hc-ink">Processing payment...</p>
                      ...
                      {[
                        ...
                        <div key={label} className="flex items-center gap-3">
                          {checklist[i] ? (
                            ...
                            <Circle size={18} className="shrink-0 text-hc-ink-4" />
                          )}
                          <span className={`text-sm ${checklist[i] ? 'text-hc-ink font-medium' : 'text-hc-ink-3'}`}>
                            {label}
                          </span>
                        </div>
                      ))}
                    </div>
                    <div className="mx-auto mt-5 flex items-center justify-center gap-1.5 text-[11px] text-hc-ink-3">
                ...
                {step === STEP.success && (
                  <div className="py-4 text-center">
                    ...
                    <p className="mt-4 font-display text-lg font-bold text-hc-ink">Payment Successful</p>
                    <p className="mt-1 text-sm text-hc-ink-2">
                      ${value.toFixed(2)} added to HandyConnect wallet
                    </p>
                    <div className="mt-3 rounded-lg bg-hc-page px-3 py-2 text-xs text-hc-ink-2">
                      Reference: <span className="font-mono font-semibold text-hc-ink">{reference}</span>
                    </div>
                    <p className="mt-3 text-[11px] text-hc-ink-3">
                ...
                {step === STEP.failed && (
                  <div className="py-4 text-center">
                    ...
                    <p className="mt-4 font-display text-lg font-bold text-hc-ink">Payment Failed</p>
                    <p className="mt-1 text-sm text-hc-ink-2">
                      Maximum OTP attempts exceeded. Payment was not processed.
                    </p>
                    <p className="mt-2 text-xs text-hc-ink-3">Wallet balance unchanged.</p>
                    <div className="mt-5 flex gap-3">
                      <button
                        onClick={reset}
                        className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-hc-hairline px-4 py-3 text-sm font-bold text-hc-accent-strong transition-colors hover:bg-hc-page"
                      >
                        Cancel
                      </button>
                      <button
