...
export default function StoryComposer({ open, onClose, uid, displayName, photoURL }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={handleClose}>
      <div className="w-full max-w-md rounded-3xl border border-gray-200 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-6 py-5">
          <div className="flex items-center gap-3">
            <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-tr from-orange-500 to-amber-400 p-[2.5px]">
              <img
            ...
            <div>
              <h2 className="font-display text-lg font-extrabold text-gray-900">New story</h2>
              <p className="mt-0.5 text-xs text-gray-500">Visible for 24 hours</p>
            </div>
          </div>
        ...
        <div className="p-6">
          {preview ? (
            ...
            <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 py-10 text-center transition-colors hover:border-orange-400 hover:bg-orange-50/40">
              <span className="flex h-12 w-12 items-center justify-center rounded-full bg-orange-100 text-orange-600">
                <Camera size={20} />
              </span>
              <span className="text-sm font-bold text-gray-700">Tap to add a photo</span>
              <span className="text-xs text-gray-400">Work moments, before &amp; afters, jobs in progress</span>
              <input type="file" accept="image/*" onChange={pick} className="hidden" />
            </label>
          )}

          <label className="mb-1.5 mt-5 block text-xs font-bold uppercase tracking-wider text-gray-500">Caption (optional)</label>
          <input
            value={caption}
