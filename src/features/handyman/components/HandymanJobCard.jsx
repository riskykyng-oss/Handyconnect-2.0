...
export default function HandymanJobCard({ job, client, onStart, onComplete, onRequestPayment, onUpdateProgress, onChat, onNavigate }) {
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={clsx('flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ring-1', meta.chip)}>
          <span className={clsx('h-1.5 w-1.5 rounded-full', meta.dot)} /> {meta.label}
        </span>
      </div>

      {/* Title */}
      <p className="mt-3 text-base font-bold text-gray-900 dark:text-white">{job.title || 'Untitled job'}</p>
