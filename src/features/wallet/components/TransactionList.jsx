...
export default function TransactionList({ transactions, onOpen, emptyText = 'No transactions yet', limit: max }) {
  ...
  if (transactions.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-100 p-10 text-center">
    ...
    <div className="space-y-4">
      {visible.map(([day, txs]) => (
        <div key={day}>
          ...
          <div className="divide-y divide-gray-200/60 rounded-2xl border border-gray-200 bg-gray-100 shadow-sm">
            {txs.map((tx) => {
              ...
              return (
                <button
                  key={tx.id}
                  onClick={() => onOpen?.(tx)}
                  className="flex w-full items-center gap-3 p-4 text-left transition-colors hover:bg-gray-200/60"
                >
                  <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${credit ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-500'}`}>
                    {credit ? <ArrowDownRight size={17} /> : <ArrowUpRight size={17} />}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-gray-900">{tx.description || 'Wallet transaction'}</p>
                    <p className="mt-0.5 text-xs text-gray-400">{format(normalizeDate(tx.createdAt), 'MMM d · h:mm a')}</p>
                  </div>
                  <div className="shrink-0 text-right">
                    <p className={`font-display text-sm font-extrabold ${credit ? 'text-emerald-600' : 'text-gray-900'}`}>
