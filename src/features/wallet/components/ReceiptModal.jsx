...
export default function ReceiptModal({ tx, onClose }) {
  ...
  return (
    ...
    >
      <div className="p-5">
        <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-5 text-center dark:border-gray-700 dark:bg-gray-800/60">
          <p className="text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Amount</p>
          <p className={`mt-1 font-display text-4xl font-extrabold ${credit ? 'text-emerald-600' : 'text-gray-900 dark:text-white'}`}>
        ...
        <dl className="mt-4 space-y-2.5 text-sm">
          <div className="flex items-center justify-between gap-3">
            ...
            <dd className="truncate text-right font-semibold text-gray-900 dark:text-white">{tx.description || '—'}</dd>
          </div>
          <div className="flex items-center justify-between gap-3">
            <dt className="text-gray-400">Date</dt>
            <dd className="font-semibold text-gray-900 dark:text-white">{format(date, 'MMM d, yyyy · h:mm a')}</dd>
          </div>
          <div className="flex items-center justify-between gap-3">
          ...
          {tx.category && (
            <div className="flex items-center justify-between gap-3">
              ...
              <dd className="font-semibold text-gray-900 dark:text-white">{tx.category}</dd>
            </div>
          )}
        </dl>
        ...
        <div className="mt-5 grid grid-cols-2 gap-2">
          ...
          <button
            onClick={share}
            className="flex items-center justify-center gap-1.5 rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-bold text-white transition-colors hover:bg-orange-600"
          >
