  ...
  return (
    <div className="space-y-6 pb-24 lg:pb-0">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Manage your payments, refunds and receipts.</p>
      </div>
        ...
        <>
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div className="flex items-center gap-2 text-xs font-medium text-gray-500 dark:text-gray-400">
              <Wallet size={13} className="text-orange-500" /> Wallet Balance
            </div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 border-t border-gray-200/70 pt-4 dark:border-gray-700">
              <div>
                <p className="text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Available Credits</p>
                <p className="mt-1 font-display text-lg font-extrabold text-emerald-600 dark:text-emerald-400">${credits.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Pending Refunds</p>
                <p className="mt-1 font-display text-lg font-extrabold text-amber-600 dark:text-amber-400">${pendingRefunds.toFixed(2)}</p>
              </div>
            </div>
          </div>
          ...
          <SectionCard title="Recent Transactions" subtitle="Your latest activity" action={
            <button onClick={() => setTab('history')} className="flex items-center gap-1 text-xs font-bold text-orange-500 hover:text-orange-600">
          ...
          <SectionCard title="Payment Methods" subtitle="Cards and mobile money" action={
            <button onClick={addMethod} className="flex items-center gap-1 rounded-lg bg-orange-50 px-3 py-1.5 text-xs font-bold text-orange-600 transition-colors hover:bg-orange-100">
              <Plus size={12} /> Add
            </button>
          }>
            <div className="space-y-2">
              ...
              {methods.map((m) => (
                <div key={m.id} className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-200/40 p-3">
                  <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-100 text-orange-500 shadow-sm">
                    <CreditCard size={15} />
                  </span>
            ...
            <div className="space-y-2">
              ...
              {refunds.map((r) => (
                <div key={r.id} className="flex items-center gap-3 rounded-xl border border-gray-200 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-gray-900">{r.description || 'Refund'}</p>
            ...
            <div className="flex items-end gap-3">
              {spendingByMonth.map((m) => (
                <div key={m.label} className="flex flex-1 flex-col items-center gap-1.5">
                  ...
                  <div className="flex h-24 w-full items-end justify-center rounded-t-lg bg-orange-500/10">
                    <div className="w-full rounded-t-lg bg-orange-500 transition-all" style={{ height: `${Math.max(8, (m.total / maxSpend) * 100)}%` }} />
                  </div>
              ...
              <div className="mt-5 border-t border-gray-200/70 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map(([cat, total]) => (
                    <span key={cat} className="flex items-center gap-1.5 rounded-lg bg-gray-200/60 px-3 py-1.5 text-xs font-bold text-gray-600">
                      <TrendingDown size={12} className="text-orange-500" /> {cat} · ${Number(total).toFixed(0)}
                    </span>
                  ))}
                </div>
              </div>
        ...
        <>
          ...
          <SectionCard title="Wallet Settings" subtitle="Preferences">
            <div className="flex items-center gap-3 rounded-xl border border-gray-200 p-3.5">
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600"><ShieldCheck size={15} /></span>
